...
suppressPackageStartupMessages(library("genoPlotR"))
...
#---------------------------------------------------------------
#--------------------------------------------------------------
print("0. Preparation step, define CAZy categories ")
#---preparation-----------------------------------------------------------------------------------------------------------------------
...
#i.e., the folder with only the '*.out.dm.ps' files, in my case, the folder name is "dbcan_results"
...
print("STEP1. read in the .out.dm.ps files in the folder named as [dbCAN_results] generated from the dbCAN annotation platform ")

cazy_in_genomes<-dir(path="01_data")

#----------------------------------------------------------------
#step2: summarize abundances of each CAZy module identified in each genome 

print("STEP2. summarize abundances of each CAZy module identified in each genome")

cazy_abun_in_genomes<-data.frame(cazy_module=c(NA))

for (i in 1:length(cazy_in_genomes))
{
  temp<-read.delim(file=paste("01_data/",cazy_in_genomes[i],sep=""),sep='\t',header = F,stringsAsFactors = F)
...
#------------------------------------------------------------------------------------------------
print("STEP3. summarize abundances of CAZy module in each CAZy category as defined at the beginning of this workflow")
...
print ("writing outputfile1: summary on abundances of each CAZy module, and abundances of CAZy modules in each category, annotated in each genome")

write.table(seq_cazy_abun_in_genomes_1,file=(paste("03_output/","summary_cazy_in_genomes.csv",sep="")),row.names = F, col.names = T, sep = ',', quote=F)

print("dim outputfile1")
dim(seq_cazy_abun_in_genomes_1)

#-----------------------------------------------------------------------------------------------------------------------------------------------------
print("STEP4. Identify genomes harboring both exoglucanase and endoglucase GH modules (Group I)
      and categorize them into 6 genotype groups")

#Categorization of genomes in Group I
#Step3':
...
for (i in 1:length(exo_endo))
{
  ...
  tmp<-read.delim(file=paste("01_data/", exo_endo[i], ".faa.out.dm.ps", sep=""), sep='\t',header = F,stringsAsFactors = F)
...
print("writing outputfile2: genotype categorization of genomes preliminarily assigned into GroupI")

#write.table(GroupI_CAZyinfo_complete_1, file="GroupI_genome_categorization.csv", sep=",", row.names=F, col.names=T, quote=F)
write.table(GroupI_CAZyinfo_complete_1, file=(paste("03_output/","GroupI_genome_categorization.csv",sep="")), sep=",", row.names=F, col.names=T, quote=F)
print("dim outputfile2")
...
print("STEP5. Identify genomes harboring both exoglucanase and endoglucase GH modules (Group I)
      and categorize them into 6 genotype groups")
...
{
  tmp<-read.delim(file=paste("01_data/", groupII[i], ".faa.out.dm.ps", sep=""), sep='\t',header = F,stringsAsFactors = F)
...
print("writing outputfile3: genotype categorization of genomes preliminarily assigned into GroupII")

#write.table(groupII_CAZyinfo_complete_1,file="GroupII_genome_categorization.csv",sep=',',row.names = F,col.names = T)
write.table(groupII_CAZyinfo_complete_1,file=(paste("03_output/","GroupII_genome_categorization.csv",sep="")),sep=',',row.names = F,col.names = T)
print("dim outputfile3")
dim(groupII_CAZyinfo_complete_1)
#-----------------END------------------------------------------------------------------------------------------------
print("STEP6. genoplot, plotting the arrangemen of CAZy modules along genes in each genome")

#dbCAN_results_data_path = as.character("01_data/dbCAN_results/")
#setwd(dbCAN_results_data_path)
#getwd()
#cazy_in_genomes<-dir(path="01_data")

cazy_in_genomes_list_0<-dir(path="01_data")
#cazy_in_genomes_list_0
...
{
  temp<-read.delim(file = paste("01_data/",cazy_in_genomes_list_0[i],sep=""),header = F, sep='\t')
...
for (j in 1:length(cazy_in_genomes_list))
{
  ...
  pdf(file=paste("03_output/",cazy_in_genomes_list[j],".pdf",sep=""),height=130,width=30)
