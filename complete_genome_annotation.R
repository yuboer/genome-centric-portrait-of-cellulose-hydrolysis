...
exo<-c('GH6','GH9','GH48')
endo<-c(GH5,'GH5','GH7','GH8','GH12','GH44','GH45','GH51','GH61','GH124')
...
#2670
df_merge<-merge(df_mod, Abundance_of_cazy_gene_exo_endo, by.x="acc", by.y="bin",all=F)
dim(df_merge)
#[1] 264  59
df_merge[,38:59]<-apply(df_merge[,38:59],2, as.numeric)
cazy_gene_in_assembly_unit<-aggregate(df_merge[,38:59], by=list(df_merge$Assembly.Accession),
                            function(x) sum(x, na.rm = T))

write.table(cazy_gene_in_assembly_unit,file="cazy_gene_in_assembly_unit.csv",sep=",",col.names = T, row.names = F)
...
#write.table(cazy_module_in_assembly_unit,file="cazy_module_in_assembly_unit.csv",sep=",",col.names = T, row.names = F)
#--------------------------------------------------------------------------------------------------
#plasmid
colnames(df_merge)
#df_merge$Plasmids.INSDC
#df_merge$Plasmids.RefSeq
#df_merge$desc
#
#either
plasmid_cazy_info_refseq<-df_merge[!(is.na(df_merge$Plasmids.RefSeq)),]
#or
plasmid_cazy_info_refseq<-df_merge[grep("NC_", df_merge$Plasmids.RefSeq),]
#dim(plasmid_cazy_info_refseq)
#[1] 51 56
plasmid_cazy_info_desc<-df_merge[grep("plasmid", df_merge$desc),]
dim(plasmid_cazy_info_desc)
#[1]  1 56
write.table(plasmid_cazy_info_refseq, file="plasmid_cazygene_types.csv",sep=",", col.names = T, row.names = F)
#--------------------------
#chromosome
chromosome_cazy_info_desc<-df_merge[grep("chromosome", df_merge$desc),]
dim(chromosome_cazy_info_desc)
#[1]  151 56
#other
others_cazy_info_desc<-df_merge[!(grep("chromosome|plasmid", df_merge$desc)),]
dim(others_cazy_info_desc)
#------------------------------------------------------------------------------------------------
#cazy_gene_in_assembly_unit<-aggregate(df_merge[,38:59], by=list(df_merge$Assembly.Accession),
#                                      function(x) sum(x, na.rm = T))
...
GroupI_genome_categorization<-cazy_gene_in_assembly_unit
...
df_merge<-merge(df_mod, Abundance_of_cazy_gene_groupII, by.x="acc", by.y="bin",all=F)
dim(df_merge)
#[1] 264  59
df_merge[,38:59]<-apply(df_merge[,38:59],2, as.numeric)
groupII_cazy_gene_in_assembly_unit<-aggregate(df_merge[,38:59], by=list(df_merge$Assembly.Accession),
                                              function(x) sum(x, na.rm = T))
