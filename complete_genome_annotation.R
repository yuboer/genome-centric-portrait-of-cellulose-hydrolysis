...
oligosaccharidase<-c('GH1','GH2','GH3','GH13',GH13,'GH29','GH30', GH30,'GH35','GH38','GH39','GH42','GH43', GH43,'GH47','GH52','GH59','GH92','GH94','GH116','GH120','GH125')
...
#D:\papers_in_progress\to finish\HKU_cellulolytic_genomes\BMC Genomics\Environmental microbiome_submission\Appendix files
dbCAN_results_data_path = as.character("D:/papers_in_progress/to finish/HKU_cellulolytic_genomes/BMC Genomics/Environmental microbiome_submission/Appendix files/dbcan_results/")
...
write.table(cazy_abun_in_genomes_1,file="cazy_abun_in_genomes_1.tab",col.names = T, row.names = T, sep="\t",quote=F)
...
#complete genomes with plasmid and chromose to be aggregated
df_mod<-read.delim(file=("df_mod.tab"), sep="\t", header = T, stringsAsFactors = F)
#length(unique(df_mod$Assembly.Accession))
#2670
#library(tidyverse)
#df_mod<-df_mod %>% distinct(acc, .keep_all = T)

#dim(df_mod)
#[1] 4905   37
df_merge_cazy<-merge(df_mod,seq_cazy_abun_in_genomes_1,by.x="acc", by.y="genome",all=F )
dim(df_merge_cazy)
#[1] 3713  367
df_merge_cazy[,38:367]<-apply(df_merge_cazy[,38:367],2,as.numeric)

cazy_module_in_assembly_unit<-aggregate(df_merge_cazy[,38:367],by=list(df_merge_cazy$Assembly.Accession),
                                        function(x) sum(x, na.rm = T))


write.table(cazy_module_in_assembly_unit,file="cazy_module_gene_in_assembly_unit.csv",sep=",",col.names = T, row.names = F)
...
exo_endo_harboring_genomes<-cazy_module_in_assembly_unit[cazy_module_in_assembly_unit$sum_exo>=1 & cazy_module_in_assembly_unit$sum_endo>=1,]
dim(exo_endo_harboring_genomes)
#[1] 509 331
#check there is genomes harboring both exo and endo glucanase GH modules
other_genomes<-cazy_module_in_assembly_unit[cazy_module_in_assembly_unit$sum_exo==0 | cazy_module_in_assembly_unit$sum_endo==0,]
dim(other_genomes)
#[1] 2133  331
...
exo_endo_harboring_genomes_nc_list<-df_mod$acc[df_mod$Assembly.Accession %in% exo_endo_harboring_genomes$Group.1]
length(exo_endo_harboring_genomes_nc_list)
#1031

other_genomes_nc_list<-df_mod$acc[df_mod$Assembly.Accession %in% other_genomes$Group.1]
length(other_genomes_nc_list)
#3830

length(unique(df_mod$acc))
#4904
#4861=1031+3830
dim(df_mod)
#[1] 4905   
...
exo_endo<-exo_endo_harboring_genomes_nc_list[exo_endo_harboring_genomes_nc_list %in% cazy_in_genomes]
length(exo_endo_harboring_genomes_nc_list)
#1031
length(exo_endo)
#847
...
#
Abundance_of_cazy_gene_exo_endo<-data.frame("bin"=exo_endo_harboring_genomes_names)
#A-1
...
#exo_endo_harboring_genomes_names[121]
#[1] "NC_016609"
#####
#j=5, i=121
for (j in 1: length(exo_endo_harboring_genomes_names))
{
  ...
  for (i in 1:length(uni_query_id_in_tmp0))
  {
    ...
    {
      Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"attach_scaffold"]=Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"attach_scaffold"]+1
    }
    ...
    {
      Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"attach_scaffold_0"]=Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"attach_scaffold_0"]+1
    }
    ## A2-s attached cellulosome_adhesin_tail:     coh>=1, slh>=1
    ...
    {
      Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"attach_scaffold_0t"]=Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"attach_scaffold_0t"]+1
    }
    ...
    {
      Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"free_scaffold"]=Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"free_scaffold"]+1
    }
    ...
    {
      Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"SLH_cCBM"]=Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"SLH_cCBM"]+1
    }
    ...
    {
      Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"cGH_cCBM"]=Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"cGH_cCBM"]+1
    }
    #cGH_otherCBM
    ...
    {
      Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"cGH_otherCBM"]=Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"cGH_otherCBM"]+1
    }
    ...
    {
      Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"cGH_CBM"]=Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"cGH_CBM"]+1
    }
    ...
    {
      Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"GH_CBM"]=Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"GH_CBM"]+1
    }
    ...
    {
      Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"solitude_cGH"]=Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"solitude_cGH"]+1 
    }
    ...
    {
      Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"LPMO_CBM"]=Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"LPMO_CBM"]+1
    }
    ...
    {
      Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"solitude_LPMO"]=Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"solitude_LPMO"]+1 
    }
    ...
    {
      Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"exo_cCBM"]=Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"exo_cCBM"]+1 
    }
    
    #oligosaccharidase
    if (sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% oligosaccharidase])>=1)
    {
      Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"oligosaccharidase"]=Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"oligosaccharidase"]+1 
    }
    ######
    #SLH_cohesin
    ...
    {
      Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"SLH_cohesin"]=Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"SLH_cohesin"]+1 
    }
    #exo_dok
    ...
    {
      Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"exo_dok"]=Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"exo_dok"]+1 
    }
    #endo_dok
    ...
    {
      Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"endo_dok"]=Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"endo_dok"]+1 
    }
    #xylanase_GH_dok
    ...
    {
      Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"xylanase_GH_dok"]=Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"xylanase_GH_dok"]+1 
    }
    ...
    {
      Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"oligosaccharidase_dok"]=Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"oligosaccharidase_dok"]+1 
    }
    #LPMO_dok 
    ...
    {
      Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"LPMO_dok"]=Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"LPMO_dok"]+1 
    }
    #otherGH_dok
    ...
    {
      Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"otherGH_dok"]=Abundance_of_cazy_gene_exo_endo[Abundance_of_cazy_gene_exo_endo$bin %in% exo_endo_harboring_genomes_names[j],"otherGH_dok"]+1 
    }
  }
}
...
exo_endo_df_merge<-merge(df_mod, Abundance_of_cazy_gene_exo_endo, by.x="acc", by.y="bin",all=F)
length(unique(df_mod$Assembly.Accession))
#[1] 2670
dim(exo_endo_df_merge)
#[1] [1] 847  59
exo_endo_df_merge[,38:59]<-apply(exo_endo_df_merge[,38:59],2, as.numeric)
exo_endo_cazy_gene_in_assembly_unit<-aggregate(exo_endo_df_merge[,38:59], by=list(exo_endo_df_merge$Assembly.Accession),
                                               function(x) sum(x, na.rm = T))

#write.table(exo_endo_cazy_gene_in_assembly_unit,file="exo_endo_cazy_gene_in_assembly_unit.csv",sep=",",col.names = T, row.names = F)
dim(exo_endo_cazy_gene_in_assembly_unit)
#[1] 509  23

colnames(exo_endo_cazy_gene_in_assembly_unit)
length(unique(exo_endo_cazy_gene_in_assembly_unit$Group.1))
#[1] 509
#seq_cazy_abun_in_genomes_1

#cazy_gene_in_assembly_unit<-aggregate(df_merge[,38:59], by=list(df_merge$Assembly.Accession),
#                                      function(x) sum(x, na.rm = T))

#temp5<-aggregate(CAZy_harboring_uniq_NC_complete_info[,2:324],by=list(CAZy_harboring_uniq_NC_complete_info$Assembly.Accession),
#                 function(x) sum(x,na.rm=T))
######

GroupI_genome_categorization<-exo_endo_cazy_gene_in_assembly_unit
...
#
GroupI_CAZyinfo_complete<-merge(GroupI_genome_categorization,cazy_module_in_assembly_unit,by="Group.1",all=F)
dim(GroupI_CAZyinfo_complete)
#[1] 491 354
#new_df <- df %>%
...
colnames(GroupI_CAZyinfo_complete_1)[2]<-c("Assembly.Accession")
dim(GroupI_CAZyinfo_complete_1)
#[1] 509 354

length(unique(GroupI_CAZyinfo_complete_1$Assembly.Accession))
#[1] 509
colnames(df_mod)
dim(df_mod)
#acc_taxon<-df_mod[,c("Assembly.Accession","phylum", "class", "order", "family", "genus","species", "strain", "FTP.path","Reference")]
acc_taxon <- df_mod %>% distinct(Assembly.Accession, .keep_all = T)
#dim(acc_strain_1)
GroupI_CAZyinfo_complete_2<-merge(GroupI_CAZyinfo_complete_1,acc_taxon, by="Assembly.Accession",all=F)
dim(GroupI_CAZyinfo_complete_2)
#[1]509 390
write.table(GroupI_CAZyinfo_complete_2,file="GroupI_taxon_CAZyinfo_complete.csv",sep=',',row.names = F,col.names = T)
...
groupII<-other_genomes_nc_list[other_genomes_nc_list %in% cazy_in_genomes]
length(other_genomes_nc_list)
#[1] 3830
length(groupII)
#[1] 2866
...
#
Abundance_of_cazy_gene_groupII<-data.frame("bin"=groupII_genomes_names)
#A-1
...
for (j in 1: length(groupII_genomes_names))
{
  ...
  for (i in 1:length(uni_query_id_in_tmp0))
  {
    ...
    if (tmp_cazy_df_expan[tmp_cazy_df_expan$tmp_cazy %in% 'cohesin','Freq']>=3 & tmp_cazy_df_expan[tmp_cazy_df_expan$tmp_cazy %in% 'SLH','Freq']>=1)
    {
      Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"attach_scaffold"]=Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"attach_scaffold"]+1
    }
    # A2 attached cellulosome coh>=3, slh>=1, dok>=1
    #if (tmp_cazy_df_expan[tmp_cazy_df_expan$tmp_cazy %in% 'cohesin','Freq']>=3 & tmp_cazy_df_expan[tmp_cazy_df_expan$tmp_cazy %in% 'SLH','Freq']>=1 & tmp_cazy_df_expan[tmp_cazy_df_expan$tmp_cazy %in% 'dockerin','Freq']>=1)
    #{
    ...
    if (tmp_cazy_df_expan[tmp_cazy_df_expan$tmp_cazy %in% 'cohesin','Freq']>=3 & tmp_cazy_df_expan[tmp_cazy_df_expan$tmp_cazy %in% 'dockerin','Freq']>=1 & tmp_cazy_df_expan[tmp_cazy_df_expan$tmp_cazy %in% 'SLH','Freq']==0)
    {
      Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"attach_scaffold_0"]=Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"attach_scaffold_0"]+1
    }
    ## A2-s attached cellulosome_adhesin_tail:     coh>=1, slh>=1
    if (tmp_cazy_df_expan[tmp_cazy_df_expan$tmp_cazy %in% 'cohesin','Freq']>=1 & tmp_cazy_df_expan[tmp_cazy_df_expan$tmp_cazy %in% 'SLH','Freq']>=1)
    {
      Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"attach_scaffold_0t"]=Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"attach_scaffold_0t"]+1
    }
    
    # A3 free cGH:     coh>=3, dok==0, slh==0
    if (tmp_cazy_df_expan[tmp_cazy_df_expan$tmp_cazy %in% 'cohesin','Freq']>=3 & tmp_cazy_df_expan[tmp_cazy_df_expan$tmp_cazy %in% 'dockerin','Freq']==0 & tmp_cazy_df_expan[tmp_cazy_df_expan$tmp_cazy %in% 'SLH','Freq']==0)
    {
      Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"free_scaffold"]=Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"free_scaffold"]+1
    }
    # B SLH_cCBM(_gh):      slh>=1, sum(selcbm)>=1
    if (tmp_cazy_df_expan[tmp_cazy_df_expan$tmp_cazy %in% 'SLH','Freq']>=1 & sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% cCBM])>=1)
    {
      Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"SLH_cCBM"]=Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"SLH_cCBM"]+1
    }
    # B' SLH_CBM(_gh):      slh>=1, sum(cbm)>=1
    if (tmp_cazy_df_expan[tmp_cazy_df_expan$tmp_cazy %in% 'SLH','Freq']>=1 & sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% all_CBM])>=1)
    {
    ...
    if (sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% cGH])>=1 & sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% cCBM])>=1)
    {
      Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"cGH_cCBM"]=Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"cGH_cCBM"]+1
    }
    #cGH_otherCBM
    if (sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% cGH])>=1 & sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% other_CBM])>=1)
    {
      Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"cGH_otherCBM"]=Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"cGH_otherCBM"]+1
    }
    # C' cGH_cbm(_gh):     sum(slegh>=1, sum(cbm)>=1
    if (sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% cGH])>=1 & sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% all_CBM])>=1)
    {
      Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"cGH_CBM"]=Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"cGH_CBM"]+1
    }
    # C' GH_cbm(_gh):     sum(gh>=1, sum(cbm)>=1
    if (sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% all_GH])>=1 & sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% all_CBM])>=1)
    {
      Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"GH_CBM"]=Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"GH_CBM"]+1
    }
    # D solitude_cGH(_slh):  sum(slegh>=1, sum(selcbm)==0
    if (sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% cGH])>=1 & sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% cCBM])==0)
    {
      Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"solitude_cGH"]=Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"solitude_cGH"]+1 
    }
    # C' LPMO_cbm(CBM_LPMO):     sum(LPMO>=1, sum(allcbm)>=1
    if (sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% LPMO])>=1 & sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% all_CBM])>=1)
    {
      Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"LPMO_CBM"]=Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"LPMO_CBM"]+1
    }
    # D' solitude_LPMO(LPMO):  sum(LPMO>=1, sum(allcbm)==0
    if (sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% LPMO])>=1 & sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% all_CBM])==0)
    {
      Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"solitude_LPMO"]=Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"solitude_LPMO"]+1 
    }
    
    #exo-CBM
    if (sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% exo])>=1 & sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% cCBM])>=1)
    {
      Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"exo_cCBM"]=Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"exo_cCBM"]+1 
    }
    
    #oligosaccharidase
    if (sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% oligosaccharidase])>=1)
    {
      Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"oligosaccharidase"]=Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"oligosaccharidase"]+1 
    }
    ######
    #SLH_cohesin
    if (tmp_cazy_df_expan[tmp_cazy_df_expan$tmp_cazy %in% 'SLH','Freq']>=1 & tmp_cazy_df_expan[tmp_cazy_df_expan$tmp_cazy %in% 'cohesin','Freq']>=1)
    {
      Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"SLH_cohesin"]=Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"SLH_cohesin"]+1 
    }
    #exo_dok
    if (tmp_cazy_df_expan[tmp_cazy_df_expan$tmp_cazy %in% 'dockerin','Freq']>=1 & sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% exo])>=1)
    {
      Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"exo_dok"]=Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"exo_dok"]+1 
    }
    #endo_dok
    if (tmp_cazy_df_expan[tmp_cazy_df_expan$tmp_cazy %in% 'dockerin','Freq']>=1 & sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% endo])>=1)
    {
      Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"endo_dok"]=Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"endo_dok"]+1 
    }
    #xylanase_GH_dok
    if (tmp_cazy_df_expan[tmp_cazy_df_expan$tmp_cazy %in% 'dockerin','Freq']>=1 & sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% xylanase_GH])>=1)
    {
      Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"xylanase_GH_dok"]=Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"xylanase_GH_dok"]+1 
    }
    #oligosaccharidase_dok
    if (tmp_cazy_df_expan[tmp_cazy_df_expan$tmp_cazy %in% 'dockerin','Freq']>=1 & sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% oligosaccharidase])>=1)
    {
      Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"oligosaccharidase_dok"]=Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"oligosaccharidase_dok"]+1 
    }
    #LPMO_dok 
    if (tmp_cazy_df_expan[tmp_cazy_df_expan$tmp_cazy %in% 'dockerin','Freq']>=1 & sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% LPMO])>=1)
    {
      Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"LPMO_dok"]=Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"LPMO_dok"]+1 
    }
    #otherGH_dok
    if (tmp_cazy_df_expan[tmp_cazy_df_expan$tmp_cazy %in% 'dockerin','Freq']>=1 & sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% other_GH])>=1)
    {
      Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"otherGH_dok"]=Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"otherGH_dok"]+1 
    }
    #Abundance_of_cazy_gene_groupII$sle_GH<-c(0)
    if (sum(tmp_cazy_df_expan$Freq[tmp_cazy_df_expan$tmp_cazy %in% sle_GH])>=1)
    {
      Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"sle_GH"]=Abundance_of_cazy_gene_groupII[Abundance_of_cazy_gene_groupII$bin %in% groupII_genomes_names[j],"sle_GH"]+1 
    }
  }
}

groupii_df_merge<-merge(df_mod, Abundance_of_cazy_gene_groupII, by.x="acc", by.y="bin",all=F)
dim(groupii_df_merge)
#[1] 3219   60
groupii_df_merge[,38:60]<-apply(groupii_df_merge[,38:60],2, as.numeric)
groupII_cazy_gene_in_assembly_unit<-aggregate(groupii_df_merge[,38:60], by=list(groupii_df_merge$Assembly.Accession),
                                              function(x) sum(x, na.rm = T))
#write.table(groupII_cazy_gene_in_assembly_unit,file="groupII_cazy_gene_in_assembly_unit.csv",sep=",",col.names = T, row.names = F)
#-------------------------------------------------------------------------------------------------
...
groupII_CAZyinfo_complete<-merge(groupII_genome_categorization,cazy_module_in_assembly_unit,by="Group.1",all=F)
dim(groupII_CAZyinfo_complete)
#[1] 2133  355

groupII_CAZyinfo_complete_1<-groupII_CAZyinfo_complete %>%
  select(category, everything())
colnames(groupII_CAZyinfo_complete_1)[2]<-c("Assembly.Accession")
dim(groupII_CAZyinfo_complete_1)
#[1] 2133  355

write.table(groupII_CAZyinfo_complete_1,file="groupII_CAZyinfo_complete_1.csv",sep=',',row.names = F,col.names = T)


GroupII_CAZyinfo_complete_2<-merge(groupII_CAZyinfo_complete_1,acc_taxon, by="Assembly.Accession",all=F)
write.table(GroupII_CAZyinfo_complete_2,file="groupII_genome_categorization.csv",sep=',',row.names = F,col.names = T)
dim(GroupII_CAZyinfo_complete_2)
#[1] 2133  391
length(unique(GroupII_CAZyinfo_complete_2$Assembly.Accession))
#2133
length(unique(df_mod$Assembly.Accession))
#2670
