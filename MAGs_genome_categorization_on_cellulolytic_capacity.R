#tips
#some Groups may have no genomes assigned
#---------------------------------------------------------------------
getwd()
...
#summary on the statistics of CAZy modules identified in each draft genome 
# abundace of CAZy modules identified in each draft genome

cazy_abun_in_bins<-data.frame(cazy_module=character())
...
#now, the number of each CAZy module identified in each MAG/bin has been summarized in 'cazy_abun_in_bins_1'
#-----------------------------------------------------------------------------
 
# the categorization of the CAZy modules were accoridng to updated info on the functions described in the CAZy database
# this part could be updated in future according to discovery of new CAZy modules or the updated function description of each module
#--------------------------------------------------------------------
...
#--------------------------------------------------------------------------------------------------------------------
#Organize a bit in presenting the information on the CAZy modules identified in each genome;
#to oresent the CAZy moudles in certain sequence, and add several more columns, e.g., the total number of exoglucanase GH modules identified in each genome
...
seq_cazy_abun_in_bins<-seq_cazy_abun_in_bins[new_colnames[new_colnames %in% colnames(seq_cazy_abun_in_bins)]]

write.table(seq_cazy_abun_in_bins,file="seq_cazy_abun_in_bins.tab",row.names = T, col.names = T, sep = '\t')
######Info on the diversity and abundance of CAZy modules identified in each genome,together with some prelimilary summary info was written into the "seq_cazy_abun_in_bins.tab" file
...
exo_endo_harboring_bins<-seq_cazy_abun_in_bins[!seq_cazy_abun_in_bins$sum_exo==0 & !seq_cazy_abun_in_bins$sum_endo==0,]
only_exo_harboring_bins<-seq_cazy_abun_in_bins[!seq_cazy_abun_in_bins$sum_exo==0 & seq_cazy_abun_in_bins$sum_endo==0,]
only_endo_harboring_bins<-seq_cazy_abun_in_bins[seq_cazy_abun_in_bins$sum_exo==0 & !seq_cazy_abun_in_bins$sum_endo==0,]
only_hemicellulase_harboring_bins<-seq_cazy_abun_in_bins[seq_cazy_abun_in_bins$sum_exo==0 & seq_cazy_abun_in_bins$sum_endo==0 & !seq_cazy_abun_in_bins$sum_hemicellulase==0,]
only_oligosaccharidase_harboring_bins<-seq_cazy_abun_in_bins[seq_cazy_abun_in_bins$sum_exo==0 & seq_cazy_abun_in_bins$sum_endo==0 & !seq_cazy_abun_in_bins$sum_oligosaccharidase==0, ]
no_carbohydrase_harboring_bins<-seq_cazy_abun_in_bins[seq_cazy_abun_in_bins$sum_exo==0 & seq_cazy_abun_in_bins$sum_endo==0 & seq_cazy_abun_in_bins$sum_hemicellulase==0 & seq_cazy_abun_in_bins$sum_oligosaccharidase==0,]
...
write.table(no_carbohydrase_harboring_bins,file = "seq_cazy_not_carbohydrates_degrading_bins.tab",sep="\t",row.names = F,col.names = T)
...
# read the info on the CAZy modules annotated in each draft genome, into a list
...
#### identify different types of CAZy harboring genes in each draft genomes and calculate the abundance of each the gene type identified
...
for (j in 1: length(exo_endo_harboring_bins_names))
{
  ...
  for (i in 1:length(uni_query_id_in_tmp0))
  {
    ...
    #different types of the CAZy modules would be identified in different genomes, by introducing 'aoo_exclu", the column bumber of each MAG would be the same, a cleaner and more unified organization
...
#the scripts below (need to scroll a bit further) is for further categorization of MAGs assigned in Group II, Group III, Group Iv, which results is not highly informative as none of them would be of cellulolytic microbes
...
#you can choose to not run the scripts below
#-------------------------------------------------------------------------
...
#> only_exo
#character(0)
#no MAGs were identified with only the exoglucanase GH modules
